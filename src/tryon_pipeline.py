...
class StableDiffusionXLInpaintPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # cloth_logo = self._encode_vae_image(cloth_logo_mask, generator=generator)
        
        if cloth_filtering:
            cloth_filters = cloth_filter # ([1, 1, 1024, 768])
            # resize the mask to latents shape as we concatenate the mask to the latents
            print(height, width)
            cloth_filter = torch.stack(
                [
                    torch.nn.functional.interpolate(cloth_filters, size=(height // 8, width // 8))
                ]
            )
            cloth_filter = cloth_filter.reshape(-1, 1, height // 8, width // 8)
            cloth_filter = cloth_filter.to(dtype = torch.float16)

            cloth = torch.cat([cloth, cloth_filter], dim=1) # ([1, 5, 128, 96])
            
        if cloth_logo_masking:
            cloth_filters = cloth_filter # ([1, 1, 1024, 768])
            # resize the mask to latents shape as we concatenate the mask to the latents
            print(height, width)
            cloth_filter = torch.stack(
            ...
            cloth = torch.cat([cloth, cloth_filter, cloth_logo], dim=1) # ([1, 9, 128, 96])
            print(cloth.shape)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print(f"reference_features shape: {(reference_features[0].shape)}")
                # print(f"reference_features shape: {(reference_features[1].shape)}")
        ...
        # if not return_dict:
        return (image, masked_image_latents)
