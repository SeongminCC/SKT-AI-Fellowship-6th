...
class VitonHDDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_pil_big = Image.open(
            os.path.join(self.dataroot, self.phase, "image", im_name)).convert("RGB").resize((768, 1024))
        ori_w, ori_h = im_pil_big.size
        image, tunnel_bbox, tunnel_info = self.pipeline.process_image(np.array(im_pil_big), expansion_ratio=1.46, prev_state=None)
        image = self.transform(Image.fromarray(image))
        # load parsing image


        mask = Image.open(os.path.join(self.dataroot, self.phase, "agnostic-mask", im_name.replace('.jpg','_mask.png'))).convert("RGB").resize(im_pil_big.size)
        cropped_agn_mask_img = self.pipeline.crop_image(np.array(mask), tunnel_bbox)
        agnostic_mask_img = self.pipeline.pad_image_to_square(cropped_agn_mask_img)
        mask = self.transform(Image.fromarray(agnostic_mask_img))
        mask = mask[:1]
        ...
        ).convert("RGB").resize(im_pil_big.size)
        cropped_pose_img = self.pipeline.crop_image(np.array(densepose_map), tunnel_bbox)
        ...
        result["cloth_pure"] = self.transform2D(cloth)
        result["cloth_filter"] = self.transform2D(fil_cloth)
...
def main():
    ...
    args = parse_args()
    accelerator_project_cnfig = ProjectConfiguration(project_dir=args.output_dir)
